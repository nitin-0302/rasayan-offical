...
export default function EntryPassModal({ registration, onClose }: EntryPassModalProps) {
  ...
  // High quality QR code URL from QRServer
  const qrCodeImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(verifyUrl)}&color=052e16&bgcolor=ffffff&margin=10`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
      <motion.div
        ...
        className="bg-slate-900 border border-emerald-500/30 rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden text-slate-100 my-auto"
      >
        ...
        <div className="p-4 sm:p-6 overflow-x-auto flex justify-center">
          <div
            ref={passRef}
            className="w-full max-w-[620px] bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-900 border-2 border-emerald-500/40 rounded-2xl p-5 sm:p-6 relative overflow-hidden shadow-2xl text-left select-none"
            style={{ minWidth: '320px' }}
          >
            {/* Holographic Watermark & Chemistry Accents */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-64 h-64 bg-teal-500/10 rounded-full blur-3xl pointer-events-none" />
            
            {/* Top Pass Header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-emerald-500/20 relative z-10">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center font-serif font-black text-white text-xl shadow-lg shadow-emerald-500/20 ring-1 ring-white/20">
                  R
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h2 className="text-lg font-serif font-bold text-white tracking-wide">RASAYAN 2026</h2>
                    <span className="bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">
                      OFFICIAL PASS
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-1 bg-emerald-950/80 border border-emerald-500/40 px-3 py-1 rounded-full text-emerald-300 text-xs font-mono font-bold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 py-5 relative z-10">
              ...
              <div className="sm:col-span-2 space-y-3.5">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-400/80 block mb-0.5">
                    Participant Name
                  </span>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {regEvents.map((e, idx) => (
                      <span
                        key={idx}
                        className="bg-slate-900/90 border border-emerald-500/30 text-emerald-200 text-[10px] font-bold px-2 py-0.5 rounded-md"
                      >
                        {e?.name || 'Event'}
                      </span>
                    ))}
                ...
                <div className="flex items-center gap-2 pt-1">
                  <span className="inline-flex items-center gap-1 bg-green-500/20 text-green-300 border border-green-500/40 px-2.5 py-0.5 rounded-full text-[10px] font-bold">
                    <CheckCircle2 className="w-3 h-3 text-green-400" /> Payment Verified
                  </span>
              ...
              <div className="flex flex-col items-center justify-center p-3 bg-white/95 border border-emerald-400/40 rounded-xl shadow-inner text-center">
                <img
                  src={qrCodeImageUrl}
            ...
            {/* Bottom Pass Footer */}
            <div className="pt-3 border-t border-emerald-500/20 flex flex-col sm:flex-row justify-between items-center gap-2 relative z-10 text-[10px] text-slate-400">
        ...
        <div className="p-4 bg-slate-950/80 border-t border-slate-800 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs bg-slate-800 hover:bg-slate-700 text-emerald-400 px-3 py-2 rounded-xl font-medium transition-colors flex items-center gap-1.5"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs bg-emerald-950 text-emerald-300 border border-emerald-500/40 hover:bg-emerald-900 px-3.5 py-2 rounded-xl font-bold transition-all flex items-center gap-1.5"
            >
            ...
            <button
              ...
              disabled={isDownloading}
              className="text-xs bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold px-4 py-2 rounded-xl shadow-lg shadow-emerald-500/20 transition-all flex items-center gap-1.5"
            >
              {isDownloading ? (
